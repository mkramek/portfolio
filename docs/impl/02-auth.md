...
2. Configure Better Auth (`app/lib/auth.ts` or equivalent) with the `magicLink`, `emailOTP`, and `passkey` plugins enabled. Wire SMTP sending (Nodemailer, per [[../arch/01-tech-stack|Tech Stack]]) into each plugin's email-sending hook, using MJML-compiled templates (author `.mjml` source files in `app/emails/`, compile to HTML at send time or as a build step — implementation's call).
