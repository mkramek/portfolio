...
2. Configure Better Auth (`app/lib/auth.ts` or equivalent) with the `magicLink`, `emailOTP`, and `passkey` plugins enabled. Wire mail sending (per [[../arch/01-tech-stack|Tech Stack]]) into each plugin's email-sending hook via a single `app/lib/email.ts` send function, using MJML-compiled templates (author `.mjml` source files in `app/emails/`, compile to HTML at send time or as a build step — implementation's call). That function should support SMTP, Gmail XOAUTH2, and Mailgun's HTTP API, selected by `MAIL_TRANSPORT` — see [[../arch/adr/015-gmail-xoauth2-transport|ADR-015]], [[../arch/adr/016-mailgun-http-api-transport|ADR-016]], and [[09-deployment|impl: deployment]] § Gmail XOAUTH2 / § Mailgun HTTP API.
